...
# 共享库(_math/_logging)位于插件目录的父目录(仓库根)
_sys_path_add(PLUGIN_DIR.parent)
# 本地安装的 kicad-python(kipy)依赖(IDE/独立调试时使用,KiCad 虚拟环境中则不启用)
for _sp in (PLUGIN_DIR.parent / "site-packages", PLUGIN_DIR / "site-packages"):
    if _sp.is_dir():
        _sys_path_add(_sp)


from kipy import kicad as kipy_kicad  # noqa: E402
from kipy.board_types import BoardSegment, Track  # noqa: E402
from kipy.geometry import Vector2  # noqa: E402

from _math import Line2D, Vec2D  # noqa: E402

# 日志记录模式与 FreeDiffPair 一致:输出到 stdout 和 plugin.log
try:
    from _logging import PrefixLogger, Config  # noqa: E402

    G_PLUGIN_LOG_FILE = f"{PLUGIN_DIR.as_posix()}/plugin.log"
    logger = PrefixLogger(
        "PathDiffGen",
        sinks=[
    ...
    with open(f"{PLUGIN_DIR.as_posix()}/plugin.log", "a") as f:
...
class KiLinePoint2D:
    ...
    def __eq__(self, object: KiLinePoint2D):
...
class KiLine2D:
    ...
    def toLine2D(self) -> Line2D:
        """转换为 _math.Line2D 向量线段"""
        return Line2D(
            Vec2D(self.start.x, self.start.y),
            Vec2D(self.end.x, self.end.y),
        )
...
def MakeDiffPl(plin: KiLinePolyline2D, clearance: int | float):
    ...
    while True:
        ...
        # 创建[当前参考线段]的平行线
        plin_curdiff_line = Line2D.make_parallel(plin_cur_line.toLine2D(), clearance)

        # 初始化起点
        if retpl.MaxLineIndex() is None:
            vec_start = plin_curdiff_line.start_pt
        ...
        # 修正[输出折线的终点]为[输出折线的末尾线段]与[当前参考线段平行线]的交点
            ...
            # 构造新的平行线 求与上一线段交点
            logger.debug(f"    当前DIFF{plin_curdiff_line}")
            logger.debug(f"    上个DIFF{retpl_endline.toLine2D()}")
            junction = Line2D.get_linear_junction(plin_curdiff_line, retpl_endline.toLine2D())
            if junction is None:
        ...
        vec_end = plin_curdiff_line.end_pt
...
def main(client: kipy_kicad.KiCad):
    ...
    track2 = TrackList[1]
    vec1 = Line2D(
        Vec2D(track1.start.x, track1.start.y),
        Vec2D(track1.end.x, track1.end.y),
    )
    vec2 = Line2D(
        Vec2D(track2.start.x, track2.start.y),
        Vec2D(track2.end.x, track2.end.y),
    )

    dp_layer = track1.layer
    dp_distance = Line2D.get_parallel_clearance(vec1, vec2, 10000000)

    # 检查差分线1 要求是平行线
    if dp_distance is None:
        logger.fatal("输入线路不平行,要求输入平行差分对")
        logger.fatal(f"  {vec1} rA={vec1.get_vec().angle()}")
        logger.fatal(f"  {vec2} rA={vec2.get_vec().angle()}")
        logger.fatal(f"  平行误差 {Vec2D.cross(vec1.get_vec(), vec2.get_vec()) / 1000000}")
        return
    ...
    # get_parallel_clearance 的符号仅表示方向(取决于两条线的先后顺序), 距离取绝对值
    dp_distance = abs(dp_distance)
...
if __name__ == "__main__":
    ...
    def _find_kicad_python() -> str | None:
        try:
            k = kipy_kicad.KiCad()
            pcbnew = k.get_kicad_binary_path("pcbnew")
            if pcbnew:
                for exe in "python.exe":
                    p = Path(pcbnew).parent / exe
                    if p.is_file():
                        return p.as_posix()
        except Exception:
            ...
    ...
    if os.environ.get("KICAD_API_SOCKET") and os.environ.get("PATHDIFFGEN_CHILD") != "1":
        ...
        def run_subprocess():
            ...
            if kicad_python is None:
                sys.exit(1)
            env = os.environ.copy()
            env["PATHDIFFGEN_CHILD"] = "1"
            ret = subprocess.run([kicad_python, str(Path(__file__).resolve())], env=env)
