...
class TrackLike(Protocol):
    ...
    def AddTo(self, board) -> None: ...
    def RemoveFrom(self, board) -> None: ...
...
class TrackPoint2i:
    ...
    class bindInfo:
        def __init__(self, pyTrack: TrackLike, ptype: int):
            self.obj: TrackLike = pyTrack
    ...
    def __init__(self, x: int | float, y: int | float, bind: bindInfo) -> None:
        ...
        self._y: int = int(y)
        # 端点绑定的线路列表
        self._bindList: List[TrackPoint2i.bindInfo] = [bind]
    ...
    def HasBind(self, obj: TrackLike, ptyle: int):
    ...
    def Update(self):
        if len(self._bindList) == 0:
            printlog.error(f"{self} 端点未绑定任何线段 不会执行任何操作")
            return
        ...
        for track in self._bindList:
            ...
            if ptype == self.TRACK_END_POINT:
                printlog.info(f"(0x{id(tobj):X}).SetEnd   ({self._x:+}, {self._y:+})")
                tobj.SetEnd(Vec2D(self._x, self._y))
                continue
            # 端点类型起点
            if ptype == self.TRACK_START_POINT:
                printlog.info(f"(0x{id(tobj):X}).SetStart ({self._x:+}, {self._y:+})")
                tobj.SetStart(Vec2D(self._x, self._y))
                continue
            printlog.error(f"<0x{id(tobj):X}> 不存在的端点类型({ptype}) 不会执行任何操作.")
        return
...
class TrackPolyline2i:
    def __init__(self, start: TrackPoint2i) -> None:
        self._i: int = 1
        self._ptList: List[TrackPoint2i] = [start]
    ...
    def AddPoint(self, pt: TrackPoint2i):
...
def ExportInfo(pyTrackList: List[TrackLike]):
    #
    printlog.info("")
    printlog.info(f"{ExportInfo.__name__}():")

    printlog.info("获取线路物理信息:")
    ...
    printlog.info(f"  铜层 {pyTrackList[0].GetLayerName()}({layer})")
    printlog.info(f"  线宽 {width}(unit)")
...
class ExportPoint_Result:
    def __init__(
        self,
        diff_tlist: List[Tuple[TrackPoint2i, TrackPoint2i]],
        share_ptlist: List[TrackPoint2i],
        share_ep_2pt: Tuple[TrackPoint2i, TrackPoint2i],
    ) -> None:
        self.diffList: List[Tuple[TrackPoint2i, TrackPoint2i]] = diff_tlist
        self.shareList: List[TrackPoint2i] = share_ptlist
        self.shareEp2x: Tuple[TrackPoint2i, TrackPoint2i] = share_ep_2pt


def ExportPoint(pyTrackList: List[TrackLike]):
    #
    all_point_list: List[TrackPoint2i] = []
    printlog.info("")
    printlog.info(f"{ExportPoint.__name__}():")

    def AddPoint(x, y, ptInfo: TrackPoint2i.bindInfo, ptList: List[TrackPoint2i]):
        for point in ptList:
            # 端点存在 绑定线路
            if point.XYEQ(x, y):
                point.AppendBind(ptInfo)
                printlog.info(f"  绑定到   {point}")
                return
            continue
        # 增加到缓存 绑定线路
        obj = TrackPoint2i(x, y, ptInfo)
        ptList.append(obj)
        printlog.info(f"  新的端点 {obj}")

    # 获取 所有线段端点 合并相同点时绑定到对应对象
    printlog.info(f"输入 线路端点检查(x{len(pyTrackList)}):")
    for track in pyTrackList:
        printlog.info(f"  {track}")
        # 导入线路的起点
        ptStart = track.GetStart()
        AddPoint(
            ptStart.x,
            ptStart.y,
            TrackPoint2i.bindInfo(track, TrackPoint2i.TRACK_START_POINT),
            all_point_list,
        )
        # 导入线路的终点
        ptEnd = track.GetEnd()
        AddPoint(
            ptEnd.x,
            ptEnd.y,
            TrackPoint2i.bindInfo(track, TrackPoint2i.TRACK_END_POINT),
            all_point_list,
        )

    printlog.info("端点捕获列表:")
    indep_point_list: List[TrackPoint2i] = []
    share_point_list: List[TrackPoint2i] = []

    # 导出 折线端点列表 独立线列表
    for point in all_point_list:
        printlog.info(f"  {point}")
    ...
    track_diff_list: List[Tuple[TrackPoint2i, TrackPoint2i]] = []

    def FindALine(ptList: List[TrackPoint2i]):
        ret: Tuple[TrackPoint2i, TrackPoint2i] | None = None
        # 在点列表中查找同一线段上的两端点
        for pt1 in ptList:
            for pt2 in ptList:
                # 跳过自己
    ...
    printlog.info("输出 参考差分线A:")
    printlog.info(f"  {track1d_2pt[0].GetBindFirst().obj}")
    ...
    if track2d_2pt is not None:
        ...
        printlog.info("输出 参考差分线B:")
        printlog.info(f"  {track2d_2pt[0].GetBindFirst().obj}")
    ...
    printlog.info(f"输出 参考单端共享端点(x{len(share_point_list)}):")
    ...
    printlog.info(f"输出 参考单端悬空端点(x{len(share_endpoint_2pt)}):")
...
class ExportLine_Result:
    def __init__(
        self,
        polyline: TrackPolyline2i,
        dReferStart: Tuple[TrackPoint2i, TrackPoint2i],
        dReferEnd: Tuple[TrackPoint2i, TrackPoint2i] | None,
    ) -> None:
        self.sReferPolyline: TrackPolyline2i = polyline
        self.dReferStart: Tuple[TrackPoint2i, TrackPoint2i] = dReferStart
        self.dReferEnd: Tuple[TrackPoint2i, TrackPoint2i] | None = dReferEnd
...
def ExportLine(obj: ExportPoint_Result):
    ...
    printlog.info("")
    printlog.info(f"{ExportLine.__name__}():")

    # 验证 参考差分线数量
    tdiff_count = len(track_diff_list)
    assert tdiff_count == 1 or tdiff_count == 2, "错误(参考差分线不存在或超出支持数量)"

    def getLineToPointDistanceSum(line: Tuple[TrackPoint2i, TrackPoint2i], pt: TrackPoint2i):
    ...
    tdiff_start: Tuple[TrackPoint2i, TrackPoint2i] = track_diff_list[0]
    tdiff_end: Tuple[TrackPoint2i, TrackPoint2i] | None = None
    ...
    # 构建 参考单端折线 起点
    pl = TrackPolyline2i(ptStart)
    printlog.info(f"输入 从点列表(x{len(share_point_list) + len(share_enpoint_2pt)})构建多边形:")
    printlog.info(f"  +起点{pl.GetPointCount()} {ptStart}")
    ...
    black_obj: TrackLike | None = None
    while True:
        # 循环保护
        loop_break -= 1
        assert loop_break > 0, "循环保护(构建折线时超出最大循环次数)"

        cur_bind: TrackPoint2i.bindInfo | None = None
        ...
        cur_point: TrackPoint2i | None = None
        for pt in share_point_list:
            ...
            pl.AddPoint(pt)
            cur_point = pt
            printlog.info(f"  +端点{pl.GetPointCount()} {pt} → {cur_pobj}")
            break
    ...
    # 插入最后的端点
    pl.AddPoint(ptEnd)
    printlog.info(f"  +终点{pl.GetPointCount()} {ptEnd} = {ptEnd.GetBindFirst().obj}")

    printlog.info("输出 参考单端折线:")
    printlog.info(f"  {pl}")

    printlog.info("输出 起始参考差分线:")
    printlog.info(f"  {tdiff_start[0]},{tdiff_start[1]}")

    if tdiff_end is not None:
        printlog.info("输出 结束参考差分线:")
        printlog.info(f"  {tdiff_end[0]},{tdiff_end[1]} ")
