...
from .TrackExport import ExportInfo_Result, TrackPolyline2i  # noqa: F401
...
class PY_PCB_TRACK:
    ...
    def RemoveFrom(self, board: pcbnew.BOARD):
        board.Remove(self.ki_pcb_track)
    ...
    def SetStartEnd(self, s: Vec2D, e: Vec2D) -> None:
        self.ki_pcb_track.SetStartEnd(XY2KiVECTOR2I(s), XY2KiVECTOR2I(e))
...
def AddCuShapeTrackLock(
    ...
    """在折线各端点上增加锁定圆(KiCad 专属装饰)."""
    printlog.info("")
    printlog.info(f"{AddCuShapeTrackLock.__name__}():")

    width = info.width - toKiUnit(0.05)

    printlog.info("增加线路锁定形状:")
