...
class Vec2D:
    ...
    def fromXYXY(start: "Vec2D", end: "Vec2D"):
        u = Vec2D(0, 0)
        u.MoveStart(start.x, start.y)
        u.MoveEnd(end.x, end.y)
        return u
    ...
    def Set(self, x: NumOrNone = None, y: NumOrNone = None):
        """设置 向量值"""
        # fmt:off
        if x: self.x = x
        if y: self.y = y
    ...
    def MoveStart(self, x: Num, y: Num):
        ...
        self.offset.SetEnd(end.x - x, end.y - y)
    ...
    def isOrthogonal(u: "Vec2D", v: "Vec2D", tol=_DEFAULT_TOL) -> bool:
        """判断正交"""
        return True if abs(Vec2D.Dot(u, v)) <= tol else False
    ...
    def isParallel(u: "Vec2D", v: "Vec2D", tol=_DEFAULT_TOL) -> bool:
        """判断平行"""
        return True if abs(Vec2D.Cross(u, v)) <= tol else False
    ...
    @staticmethod
    def GetParallelClearance(u: "Vec2D", v: "Vec2D") -> float | None:
        """
        计算 u 与 v 的平行间距 \n
        +f 投影在v正方向上 \n
        =0 垂直(不存在投影) \n
    ...
    def __str__(self) -> str:
        ...
        start = self.GetStart()
        return f"<{self.__class__.__name__} ({StrNum(self.x, DIG)}, {StrNum(self.y, DIG)}) AT({StrNum(start.x, DIG)},{StrNum(start.y, DIG)}) 0x{id(self):X}>"
